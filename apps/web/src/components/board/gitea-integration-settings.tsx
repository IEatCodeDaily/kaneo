...
import { GiteaRepositoryBrowserModal } from "@/components/board/gitea-repository-browser-modal";
...
export function GiteaIntegrationSettings({ boardId }: { boardId: string }) {
  ...
  } = useGetGiteaIntegration(boardId);
  ...
  const handleDelete = async () => {
    try {
      await deleteIntegration(boardId);
      form.reset({
        baseUrl: "",
        accessToken: "",
        repositoryOwner: "",
