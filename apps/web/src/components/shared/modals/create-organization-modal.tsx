...
function CreateOrganizationModal({ open, onClose }: CreateOrganizationModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t("common:modals.createOrganization.successToast"));
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : t("common:modals.createOrganization.errorToast"),
      );
    }
  };
            ...
            <Breadcrumb>
              <BreadcrumbList>
                <BreadcrumbItem className="text-muted-foreground font-semibold tracking-wider text-sm">
                  {t("common:modals.createOrganization.breadcrumbKaneo")}
                </BreadcrumbItem>
                <BreadcrumbSeparator />
                <BreadcrumbItem className="text-foreground font-medium text-sm">
                  {t("common:modals.createOrganization.title")}
                </BreadcrumbItem>
              </BreadcrumbList>
            </Breadcrumb>
          </DialogTitle>
          <DialogDescription className="sr-only">
            {t("common:modals.createOrganization.description")}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="space-y-4 px-6">
            ...
            <Input
              ...
              placeholder={t(
                "common:modals.createOrganization.descriptionPlaceholder",
              )}
          ...
          <DialogFooter>
            ...
            >
              {t("common:modals.createOrganization.createButton")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
