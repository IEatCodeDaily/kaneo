    ...
    () => (pr: { metadata?: { merged?: boolean; draft?: boolean } | null }) => {
      if (pr.metadata?.merged) {
        return {
          ...
          label: t("publicBoard:taskDetail.prStatusMerged"),
      ...
      if (pr.metadata?.draft) {
        return {
          ...
          label: t("publicBoard:taskDetail.prStatusDraft"),
          className: "text-muted-foreground",
        };
      }
      return {
        icon: <GitPullRequest className="w-3.5 h-3.5" />,
        label: t("publicBoard:taskDetail.prStatusOpen"),
        ...
        <div className="bg-background border border-border rounded-xl flex flex-col max-h-[85vh] shadow-lg">
          <div className="flex items-center justify-between px-4 py-3 border-b border-border/70 shrink-0">
            <div className="flex items-center gap-2 min-w-0 flex-1">
              <span className="text-sm font-medium text-muted-foreground shrink-0">
                {boardSlug.toUpperCase()}-{task.number}
              </span>
              {statusLabel ? (
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5 space-y-5">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {task.dueDate && (
                  ...
                  >
                    ...
                    <span>
                      {t("publicBoard:taskDetail.dueWithDate", {
            ...
            {labels.length > 0 && (
              <div className="space-y-2">
                <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                  {t("publicBoard:taskDetail.labels")}
                </h3>
            ...
            {externalLinks.length > 0 && (
              <div className="space-y-3">
                <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">
                  {t("publicBoard:taskDetail.externalLinks")}
                </h3>
                      ...
                      >
                        <div className="flex items-center gap-2.5 flex-1 min-w-0">
                          ...
                          <div className="flex flex-col gap-0.5 flex-1 min-w-0">
                            <span className="text-sm font-medium text-foreground truncate">
                              {issue.title ||
                                t("publicBoard:taskDetail.issueFallback")}
                            </span>
                            <span className="text-xs text-muted-foreground">
                              #{issue.externalId}
                            </span>
            ...
            <div className="grid grid-cols-2 gap-4 pt-4 border-t border-border/50">
              <div>
                <div className="text-xs text-muted-foreground mb-1">
                  {t("publicBoard:taskDetail.created")}
                </div>
              ...
              {task.dueDate && (
                <div>
                  <div className="text-xs text-muted-foreground mb-1">
                    {t("publicBoard:taskDetail.dueDateLabel")}
                  </div>
                  <div className="text-sm text-foreground">
