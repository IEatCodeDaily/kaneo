...
import { getWsUrl } from "./use-board-websocket";
...
describe("getWsUrl", () => {
  ...
  it("builds a ws:// URL from an http API base", () => {
    expect(getWsUrl("board-123")).toBe(
      "ws://localhost:1337/api/ws/board-123?windowId=test-window-id",
    );
  });
  ...
  it("builds a wss:// URL from an https API base", () => {
    ...
    expect(getWsUrl("board-123")).toBe(
      "wss://example.com/api/ws/board-123?windowId=test-window-id",
    );
  });
