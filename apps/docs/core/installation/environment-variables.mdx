...
### GitHub user delegation

GitHub user delegation is a separate authorization flow from GitHub sign-in, but both use the same OAuth App credentials. Delegation requests `repo read:user user:email` only when the user explicitly connects GitHub actions; sign-in requests only identity/email access.

Configure `GITHUB_OAUTH_CLIENT_ID` and `GITHUB_OAUTH_CLIENT_SECRET` once for both flows. Configure the OAuth App callback root as `${KANEO_API_URL}/api`; Kaneo uses `/api/auth/callback/github` for sign-in and `/api/github-delegation/callback` for delegation, requesting broader repository scopes only during explicit connection.

GitHub OAuth Apps issue refreshable tokens only when the OAuth App has **Expire user authorization tokens** enabled. Kaneo securely stores and reports expiry metadata when GitHub returns it.
