...
GitHub user delegation is deliberately separate from GitHub SSO. Create a separate GitHub OAuth App with callback URL `${KANEO_API_URL}/api/github-delegation/callback`; Kaneo requests `repo read:user user:email` so it can write repositories as the connected user.

Name | Description |
--- | --- |
| `GITHUB_DELEGATION_CLIENT_ID` | OAuth client ID for Kaneo's user-delegation flow. |
| `GITHUB_DELEGATION_CLIENT_SECRET` | OAuth client secret for Kaneo's user-delegation flow. |
