...
/**
 * Migration script to handle conversion from user_email to user_id in organization_member table.
 ...
 */
export async function migrateOrganizationUserEmail() {
  console.log(
    "🔄 Checking organization_member table for user_email to user_id migration...",
  );
  ...
  try {
    ...
    if (!exists) {
      console.log(
        "🛈 organization_member table does not exist — skipping migration.",
      );
      return;
    }
    ...
    if (hasUserEmailColumn.rows.length > 0) {
      ...
      if (hasUserIdColumn.rows.length === 0) {
        await db.execute(sql`
          ALTER TABLE "organization_member" ADD COLUMN "user_id" text;
        `);
      ...
      await db.execute(sql`
        ...
        SET "user_id" = (
          SELECT u.id
          FROM "user" u
          WHERE u.email = "organization_member"."user_email"
        )
      ...
      if (
        ...
        console.log(
          `⚠️  Found ${orphanedRecords.rows[0].count} organization_member records with invalid user_email. Removing them...`,
        );
      ...
      if (nullRecords.rows[0]?.count && Number(nullRecords.rows[0].count) > 0) {
        console.log(
          `⚠️  Found ${nullRecords.rows[0].count} organization_member records with no user identification. Removing them...`,
        );
      ...
      await db.execute(sql`
        ALTER TABLE "organization_member" DROP COLUMN "user_email";
      `);
      ...
      console.log("➕ Adding user_id column to organization_member table...");
      await db.execute(sql`
        ALTER TABLE "organization_member" ADD COLUMN "user_id" text;
      `);
    }
    ...
    if (nullUserIds.rows[0]?.count && Number(nullUserIds.rows[0].count) > 0) {
      console.log(
        `⚠️  Found ${nullUserIds.rows[0].count} organization_member records with NULL user_id. Removing them...`,
      );
    ...
    console.log("✅ Organization member migration completed successfully!");
  } catch (error) {
    console.error("❌ Error during organization member migration:", error);
    throw error;
  }
}
