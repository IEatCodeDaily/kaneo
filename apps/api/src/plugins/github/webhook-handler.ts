...
import { handleIssueDeleted } from "./webhooks/issue-deleted";
...
export function setupWebhookHandlers() {
  ...
  githubApp.webhooks.on("issues.transferred", async ({ payload }) => {
    console.log("[GitHub Webhook] Handling issues.transferred");
    try {
      await handleIssueTransferred(
        payload as Parameters<typeof handleIssueTransferred>[0],
      );
      console.log("[GitHub Webhook] issues.transferred handled successfully");
    } catch (error) {
      console.error(
        "[GitHub Webhook] issues.transferred handler error:",
        error,
      );
      throw error;
    }
  ...
  githubApp.webhooks.on("issues.deleted", async ({ payload }) => {
    console.log("[GitHub Webhook] Handling issues.deleted");
    try {
      await handleIssueDeleted(
        payload as Parameters<typeof handleIssueDeleted>[0],
      );
      console.log("[GitHub Webhook] issues.deleted handled successfully");
    } catch (error) {
      console.error("[GitHub Webhook] issues.deleted handler error:", error);
      throw error;
    }
  });
