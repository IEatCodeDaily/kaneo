...
describe("selectBoardEdges", () => {
  ...
  it("keeps the foreign parent's edges to local tasks", () => {
    expect(keptIds).toContain("r2");
    expect(keptIds).toContain("r3");
  });

  it("hides the foreign parent's children on other boards", () => {
    expect(keptIds).not.toContain("r1");
    expect(keptIds).not.toContain("r4");
  });

  it("keeps a local task's own subtasks, including cross-board ones", () => {
    expect(keptIds).toContain("r5");
    expect(keptIds).toContain("r6");
    expect(keptIds).toContain("r7");
  });
