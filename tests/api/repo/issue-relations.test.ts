...
describe("GitHub issue relations", () => {
  it("returns parent and sub-issues", async () => {
    ...
    const octokit = {
      request: vi.fn().mockResolvedValue({ data: parent }),
      paginate: vi.fn().mockResolvedValue(children),
    };
    await expect(getGitHubIssueRelations(octokit, {})).resolves.toEqual({
      parent,
  ...
  it("handles unsupported endpoints but propagates operational errors", async () => {
    ...
    const octokit = {
      request: vi.fn().mockRejectedValue(unsupported),
      paginate: vi.fn().mockRejectedValue(unsupported),
    };
    await expect(getGitHubIssueRelations(octokit, {})).resolves.toEqual({
      parent: null,
      parentSupported: false,
      subIssues: [],
      subIssuesSupported: false,
    });
    octokit.request.mockRejectedValueOnce(
      Object.assign(new Error("rate limited"), { status: 429 }),
    );
    await expect(getGitHubIssueRelations(octokit, {})).rejects.toThrow(
      "rate limited",
    );
  });
});
